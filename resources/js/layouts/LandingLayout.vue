    ...
    <v-navigation-drawer v-model="openlogindialog" app temporary right style="max-width: 450px;" width="80%">
      ...
      <v-row v-if="haveaccount">
        <v-col cols="12" md="12" class="pl-6 pr-6">
          ...
          <p class="text-center">Don't have an account? <v-btn text @click="haveaccount = false" small>Register</v-btn>
          </p>
      ...
      <v-row v-else>
        <v-col cols="12" md="12" class="pl-6 pr-6">
          ...
          <p class="text-center">Already have an account? <v-btn text @click="haveaccount = true" small>Login</v-btn>
          </p>
...
export default {
  ...
  methods: {
    ...
    register() {
      this.loading = true;
      axios.post("/api/user/signup", this.booking).then((response) => {
        if (response.data.success) {
          localStorage.setItem('userdetails', JSON.stringify(response.data.user));
          localStorage.setItem('token', response.data.token);
          this.isUserlogin = true;
