  ...
  <div v-shortkey="['ctrl', '/']" class="d-flex flex-grow-1" @shortkey="onKeyup">
    ...
    <v-navigation-drawer v-model="drawer" app floating class="elevation-1" :right="$vuetify.rtl"
      ...
      <template v-slot:prepend>
        <div class="pa-2" v-if="storeDetails.logo != null && storeDetails.logo != ''">
          <v-img :src="storeDetails.logo" max-height="48"></v-img>
        </div>
        <div class="pa-2" v-else>
          <div class="title font-weight-bold text-uppercase primary--text">{{ storeDetails.name }}</div>
    ...
    <v-app-bar app :color="isToolbarDetached ? 'surface' : undefined" :flat="isToolbarDetached"
      ...
      <v-card class="flex-grow-1 d-flex" :class="[isToolbarDetached ? 'pa-1 mt-3 mx-1' : 'pa-0 ma-0']"
        ...
        <div class="d-flex flex-grow-1 align-center">
          ...
          <!-- search input mobile -->
          <v-text-field v-if="showSearch" append-icon="mdi-close" placeholder="Search" prepend-inner-icon="mdi-magnify"
            hide-details solo flat autofocus @click:append="showSearch = false"></v-text-field>
...
export default {
  ...
  computed: {
    ...mapState('app', ['product', 'isContentBoxed', 'menuTheme', 'toolbarTheme', 'isToolbarDetached', 'storeDetails'])
  },
  methods: {
