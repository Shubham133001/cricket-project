            ...
            <v-card-text>
                <v-simple-table>
                    ...
                    slot.price }}</strong>
                        ...
                        <tbody v-else>
                            <tr>
                                <td colspan="8" class="text-center">No slots found<br /><v-btn color="primary"
                                        @click="addslot()" class="mt-2">Add New Slot</v-btn></td>
                            </tr>
                        </tbody>
                    </template>
                </v-simple-table>
...
export default {
    ...
    methods: {
        ...
        async getSlots() {
            try {
                const response = await axios.post('/api/admin/slots/list', { id: this.$route.params.id }).then(response => {
