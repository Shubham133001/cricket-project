  ...
  <div style="width: 100%">
    <v-row>
      <v-img
        ...
        :src="'/storage/images/' + selecteditem.image"
        lazy-src="https://picsum.photos/id/114/1920/450"
        cover
      >
        <v-row>
          <v-col
          ...
          <v-col cols="12" md="4">
            ...
            <v-btn color="primary" class="mt-2" @click="opendirection"
              ><v-icon small>mdi-directions</v-icon>Get Directions</v-btn
            >
          </v-col>
        </v-row>
      </v-img>
                ...
                <v-list-item-group>
                  <v-list-item v-for="(slot, index) in slots" :key="index">
                    <template v-slot:default="{ active }">
                      ...
                      <v-list-item-content
                        style="border-bottom: solid 1px #ececec"
                      >
                        <v-list-item-title>
          ...
          <v-card v-if="selection.length > 0">
                    ...
                    <v-simple-table>
                      ...
                      <tbody>
                        <tr v-for="slot in selection" :key="slot.id">
                          ...
                          <td>₹{{ slot.advanceprice }} INR</td>
                          <td>₹{{ slot.price }} INR</td>
                        </tr>
                      </tbody>
                      <tfoot>
                        <tr>
                ...
                <tr>
                  <td colspan="2">
                    ...
                    <span style="float: right"
                      >₹{{ totalprice - (advanceprice + credits) }} INR</span
                    >
                  </td>
                </tr>
                <tr>
            ...
            <v-card-actions>
              ...
              >
                ...
                <v-btn
                  color="primary"
                  v-if="isUserlogin && selection.length > 0"
                  block
                  large
                  @click.stop="bookslot"
                >
      ...
      <v-dialog v-model="showbooking" max-width="350">
        <v-card>
          <v-card-title class="text-h5"
            >Bookings <v-spacer></v-spacer
            ><v-icon dense small fab @click="showbooking = false"
              >mdi-close</v-icon
            ></v-card-title
          >
          <v-card-text>
                  ...
                  <v-list-item-title>
                    <v-avatar size="70" color="#efefef" style="float: left">
                      <v-img
                        ...
                        v-if="
                          booking.team.image != '' && booking.team.image != null
                        "
                        class="align-center"
                      />
                      <span
                        ...
                        v-else
                        >{{ booking.team.name.charAt(0) }}</span
                      >
                    </v-avatar>
                    <h3
                      style="
                      ...
                      "
                      class="mt-0 ml-1 text-h4"
                    >
                      {{ booking.team.name }}<br /><v-chip
                        color="orange"
                        dark
  ...
  methods: {
    ...
    bookfullslot(slot, event) {
      ...
      if (event) {
        ...
        let index = this.newbookings.findIndex(
    ...
    bookslot() {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success) {
            if (
              response.data.data["invoice_id"] != null &&
              response.data.data["invoice_id"] != 0
            ) {
              this.$router.push({
                name: "payinvoice",
                params: {
